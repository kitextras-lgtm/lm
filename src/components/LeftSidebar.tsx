...
function HomeIconSVG({ isHovered }: { isHovered: boolean }) {
  return (
    <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className={ICON_SIZE}>
      <rect x="12" y="20" width="24" height="26" rx="2" stroke="currentColor" strokeWidth="2.5" fill="none"/>
      <path d="M4 22L24 4L44 22" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
      <path 
...
function TalentIconSVG({ isHovered }: { isHovered: boolean }) {
  return (
    <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${ICON_SIZE} overflow-visible`}>
      ...
      <circle cx="12" cy="22" r="5" stroke="currentColor" strokeWidth="2.5" fill="none"
        style={{ opacity: 0.5, transform: isHovered ? "translateX(-3px)" : "translateX(0)", transition: "all 0.3s ease" }} />
      <path d="M4 44C4 37 7 32 12 32" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" fill="none"
        style={{ opacity: 0.5, transform: isHovered ? "translateX(-3px)" : "translateX(0)", transition: "all 0.3s ease" }} />
      <circle cx="36" cy="22" r="5" stroke="currentColor" strokeWidth="2.5" fill="none"
        style={{ opacity: 0.5, transform: isHovered ? "translateX(3px)" : "translateX(0)", transition: "all 0.3s ease" }} />
      <path d="M44 44C44 37 41 32 36 32" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" fill="none"
        style={{ opacity: 0.5, transform: isHovered ? "translateX(3px)" : "translateX(0)", transition: "all 0.3s ease" }} />
      <circle cx="24" cy="20" r="6" stroke="currentColor" strokeWidth="2.5" fill="none"
        style={{ transform: isHovered ? "scale(1.05)" : "scale(1)", transformOrigin: "24px 20px", transition: "transform 0.3s ease" }} />
      <path d="M12 44C12 35 17 30 24 30C31 30 36 35 36 44" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" fill="none" />
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${ICON_SIZE} overflow-visible`}>
      <g style={{ transform: isHovered ? "translateX(2px) rotate(0deg)" : "translateX(-3px) rotate(-2deg)", transformOrigin: "16px 24px", transition: "transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)" }}>
        <path d="M6 12C6 10.9 6.9 10 8 10H18V14C18 15.66 16.66 17 15 17C13.34 17 12 15.66 12 14V10H8C6.9 10 6 10.9 6 12V20H10C11.66 20 13 21.34 13 23C13 24.66 11.66 26 10 26H6V34C6 35.1 6.9 36 8 36H18V32C18 30.34 16.66 29 15 29C13.34 29 12 30.34 12 32V36H18V10" stroke="currentColor" strokeWidth="2.2" strokeLinejoin="round" fill="none" />
        <path d="M18 18V16C19.66 16 21 17.34 21 19V21H23C24.66 21 26 22.34 26 24C26 25.66 24.66 27 23 27H21V29C21 30.66 19.66 32 18 32V30" stroke="currentColor" strokeWidth="2.2" strokeLinejoin="round" fill="none" />
      </g>
      <g style={{ transform: isHovered ? "translateX(-2px) rotate(0deg)" : "translateX(3px) rotate(2deg)", transformOrigin: "32px 24px", transition: "transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)" }}>
        <path d="M42 12C42 10.9 41.1 10 40 10H30V14C30 15.66 31.34 17 33 17C34.66 17 36 15.66 36 14V10H40C41.1 10 42 10.9 42 12V20H38C36.34 20 35 21.34 35 23C35 24.66 36.34 26 38 26H42V34C42 35.1 41.1 36 40 36H30V32C30 30.34 31.34 29 33 29C34.66 29 36 30.34 36 32V36H30V10" stroke="currentColor" strokeWidth="2.2" strokeLinejoin="round" fill="none" />
        <path d="M30 18V16C28.34 16 27 17.34 27 19V21H25C23.34 21 22 22.34 22 24C22 25.66 23.34 27 25 27H27V29C27 30.66 28.34 32 30 32V30" stroke="currentColor" strokeWidth="2.2" strokeLinejoin="round" fill="none" />
      </g>
      <ellipse cx="24" cy="24" rx="4" ry="6" fill="currentColor" style={{ opacity: isHovered ? 0.6 : 0, transform: isHovered ? "scale(1)" : "scale(0.5)", transformOrigin: "24px 24px", transition: "all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1) 0.2s", filter: "blur(3px)" }} />
    </svg>
  );
}
...
function EarningsIconSVG({ isHovered }: { isHovered: boolean }) {
  return (
    <div className={`relative ${ICON_SIZE} overflow-visible`}>
      ...
      >
        {/* Piggy body - larger */}
        <ellipse cx="24" cy="26" rx="18" ry="14" stroke="currentColor" strokeWidth="2.5" fill="none"/>
        {/* Snout */}
        <ellipse cx="40" cy="26" rx="5" ry="4" stroke="currentColor" strokeWidth="2.5" fill="none"/>
        {/* Eyes */}
        <circle cx="39" cy="24" r="1" fill="currentColor"/>
        <circle cx="42" cy="24" r="1" fill="currentColor"/>
        {/* Ear */}
        <path d="M14 16C14 16 11 10 16 8C21 6 24 12 24 14" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" fill="none"/>
        {/* Legs */}
        <path d="M14 38V46" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
        <path d="M22 38V46" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
        <path d="M28 38V46" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
        <path d="M36 38V46" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
        {/* Coin slot */}
...
function MoreIconSVG({ isHovered }: { isHovered: boolean }) {
  ...
  return (
    <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className={ICON_SIZE}
      ...
      {dots.map((dot, i) => (
        <circle key={i} cx={dot.cx} cy={dot.cy} r="4" fill="currentColor"
...
export function LeftSidebar({ 
  ...
  return (
    <aside className="hidden lg:flex flex-col h-screen w-[240px] fixed left-0 top-0 border-r z-40" style={{ backgroundColor: 'var(--bg-sidebar)', borderColor: 'var(--border-subtle)' }}>
